(function () {
    ...
    window.createLongQuantPromiseLab = function createLongQuantPromiseLab(deps) {
        ...
        function hookOutcomeAxisGallery(focus) {
            ...
            return `<div style="margin:14px 0 10px"><div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-end;flex-wrap:wrap;margin-bottom:7px"><div><div style="font-size:12px;color:${C.text};font-weight:900">All six complete-hook embedding planes</div><div style="font-size:8.5px;color:${C.mute};margin-top:2px">Every point is one of ${Number((outcomes.hooks || []).length).toLocaleString()} hooks. X is the named frozen score; Y is its orthogonal semantic direction. Point color is the held-out observed target. A cyan ring is the current hook.</div></div><div style="font-size:8px;color:${C.dim}">Click a training point to inspect predicted versus actual without leaving this view.</div></div><div class="pl-map-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(235px,1fr));gap:8px">${axes.map(axis => {
                const validation = hookOutcomeValidation(axis.id), value = hookOutcomePayload(focus, axis.id) || {};
                const prediction = axis.id === 'survival' ? value.holdZ : axis.id === 'quality' ? value.percentile : (focus && focus.type === 'stored' ? value.predictedOOF : value.prediction);
        ...
        function retentionForecastPanel(forecast, stored = false) {
            ...
            const modeNote = normalizationAvailable
                ? `Terminal anchor ${pct(forecast.terminalRetentionPercent)} · correction at analyzed hook end ${pct((forecast.replayCorrectionPercent || []).slice(-1)[0])}`
        ...
        function survivalScoreCard(score, stored = false) {
            ...
            return card(`<div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap"><div style="min-width:280px;flex:1"><div style="font-size:8px;color:${promoted ? C.green : C.amber};font-weight:900;text-transform:uppercase">${promoted ? 'Validated hook indicator' : 'Provisional held-out coordinate · not promoted'}</div><div style="font-size:23px;color:${C.text};font-weight:900;margin-top:2px">Hook Hold ${signed(score.holdZ, 2)}σ</div><div style="font-size:9px;color:${C.dim};line-height:1.55;margin-top:4px">Zero is the mean model prediction among the current measured-hook calibration set; +1σ is one standard deviation higher on the frozen hold direction. This unbounded coordinate can remain comparable as the library grows. ${fmt(score.percentile, 1)}th is retained only as the current-library rank. ${stored ? `Measured target ${signed(score.actualHoldZ, 2)}σ · error ${signed(score.errorStandardDeviations, 2)} residual σ.` : 'A new text input has no measured outcome; the interval is empirical out-of-fold uncertainty.'}</div></div><div style="display:flex;gap:8px;flex-wrap:wrap">${stat('endpoint hold lift', `${signed(score.predictedEndpointHoldLiftPercentagePoints, 2)} pp`, C.cyan)}${stat('predicted endpoint', pct(score.predictedAdjustedRetentionAtResponseEnd), C.green)}${stat('duration baseline', pct(score.durationBaselineRetentionAtResponseEnd), C.dim)}${stat('80% empirical range', `${signed(p10z, 2)}σ to ${signed(p90z, 2)}σ`, C.purple)}${stat('response end', `${fmt(score.responseEndSeconds, 2)}s`, C.amber)}</div></div><div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin-top:9px;font-size:8.5px;line-height:1.5"><div style="border-left:2px solid ${C.cyan};padding-left:7px;color:${C.dim}"><b style="color:${C.text}">RAW COORDINATE</b><br>Predicted excess geometric carry per second after subtracting the duration-only expectation.</div><div style="border-left:2px solid ${C.purple};padding-left:7px;color:${C.dim}"><b style="color:${C.text}">STANDARDIZATION</b><br>(raw prediction − OOF mean) ÷ OOF prediction SD. Residual SD is ${fmt(residualStd, 3)} carry points/s.</div><div style="border-left:2px solid ${C.amber};padding-left:7px;color:${C.dim}"><b style="color:${C.text}">CLAIM BOUNDARY</b><br>Useful as a stable diagnostic coordinate, but not a universal hook-quality score until future-only and normalization gates pass.</div></div>`, 'margin-bottom:10px;border-color:' + (promoted ? C.green : C.amber) + '66');
        }
        ...
        function componentScoreGallery(focus) {
            ...
            const axes = [
                { id: 'hold', label: 'Hook Hold component deletion effect', note: 'X = headline score(full) minus headline score(without component), in Hook Hold σ; Y = the component semantic background coordinate. This is the same metric as the whole-hook headline.' },
            ...
            return `<div style="margin:14px 0"><div style="font-size:12px;color:${C.text};font-weight:900">Every component score in its actual cluster plane</div><div style="font-size:8.5px;color:${C.mute};line-height:1.5;margin:3px 0 9px">Each row is one scoring channel and contains all ${components.length} naturally selected components. Scroll sideways when the hook has more components. Each map compares the selected component only with training components carrying the same frozen category label; categories may repeat or be absent.</div>${componentOutcomePointInspector()}${axes.map(axis => `<section style="border-top:1px solid ${C.border};padding-top:9px;margin-top:9px"><div style="display:flex;justify-content:space-between;gap:8px;align-items:flex-end;flex-wrap:wrap;margin-bottom:6px"><div><div style="font-size:10px;color:${C.text};font-weight:900">${esc(axis.label)}</div><div style="font-size:7.5px;color:${C.mute}">${esc(axis.note)}</div></div><div style="font-size:7.5px;color:${C.cyan};font-weight:900">${components.length} COMPONENT MAPS →</div></div><div class="pl-component-map-strip" style="display:flex;gap:7px;overflow-x:auto;overscroll-behavior-x:contain;padding-bottom:7px">${components.map((component, index) => {
                ...
                const display = axis.id === 'hold' ? `${signed(value.coordinate, 2)}σ` : ['broad', 'forward'].includes(axis.id) ? `${fmt(value.percentile, 1)}th` : formatHookOutcomeValue(value.coordinate, axis.id);
        ...
        function hookScoreResultPanel(result) {
            ...
            const scorecard = result.scorecard || {}, scoreCoverage = scorecard.coverage || {};
            ...
            return `<div data-pl-hook-score-result><div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px">${stat('Hook Hold', `${signed(score.holdZ, 2)}σ`, C.amber)}${stat('endpoint lift', `${signed(score.predictedEndpointHoldLiftPercentagePoints, 2)} pp`, C.cyan)}${stat('random-fold rho', fmt(confidence.heldoutSpearman, 3), C.text)}${stat('future rho', fmt(confidence.chronologicalHeldoutSpearman, 3), C.red)}${stat('permutation p', fmt(confidence.familyCorrectedRankPermutationP, 5), C.purple)}${stat('normalization robust', confidence.normalizationRobust ? 'yes' : 'no', confidence.normalizationRobust ? C.green : C.red)}${stat('retained info', `${fmt(retained.percentile, 1)}th`, C.amber)}${stat('domain similarity', `${fmt(confidence.inDomainSimilarityPercentile, 1)}th`, Number(confidence.inDomainSimilarityPercentile) < 10 ? C.amber : C.green)}${stat('partition certainty', `${fmt(confidence.partitionScoreGapPercentile, 1)}th`, C.amber)}</div>${card(`<div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap"><div style="min-width:300px;flex:1"><div style="font-size:8px;color:${C.cyan};font-weight:900;text-transform:uppercase">Complete scoring contract</div><div style="font-size:16px;color:${C.text};font-weight:900;margin-top:3px">One hook score, ${components.length} component scores, ${pairs.length} relationship scores</div><div style="font-size:8.5px;color:${C.dim};line-height:1.55;margin-top:4px">Headline = frozen Hook Hold coordinate. Component = headline score(full) − headline score(without that exact component). Relationship = the second-order headline interaction after deleting each component and both together. Every value uses the same whole-hook model and literal counterfactual text; category-specific forward-response and direct-outcome channels remain visible as separate diagnostics.</div></div><div style="display:flex;gap:8px;flex-wrap:wrap">${stat('token coverage', scoreCoverage.tokensOwnedExactlyOnce ? 'exactly once' : 'FAILED', scoreCoverage.tokensOwnedExactlyOnce ? C.green : C.red)}${stat('components', `${scoreCoverage.componentsScored || 0}/${scoreCoverage.componentsExpected || components.length}`, C.cyan)}${stat('relationships', `${scoreCoverage.relationshipsScored || 0}/${scoreCoverage.relationshipsExpected || pairs.length}`, C.purple)}${stat('claim', String((scorecard.headlineMetric || {}).validationStatus || '').startsWith('validated') ? 'promoted' : 'diagnostic', C.amber)}</div></div>`, 'margin-bottom:10px;border-color:' + C.cyan + '66')}${survivalScoreCard(score, false)}${outcomePredictionStrip(focus)}${retentionForecastPanel(outcomes.retentionForecast, false)}
            ...
            ${card(`<div style="display:flex;justify-content:space-between;gap:8px;align-items:flex-start;flex-wrap:wrap;margin-bottom:8px"><div><div style="font-size:11px;color:${C.text};font-weight:900">Variable-count exact-cover decomposition · ${components.length} components</div><div style="font-size:8px;color:${C.mute};margin-top:2px">Every token has one owner and zero overlap. Four is the category vocabulary, not the component count. Boundaries use no outcome and no supplied example labels. Component scores below use the same Hook Hold model as the headline.</div></div><div style="font-size:8px;color:${C.amber}">top-two gap ${fmt(confidence.partitionScoreGap, 5)} · ${fmt(confidence.partitionScoreGapPercentile, 1)}th training percentile</div></div><div style="display:flex;gap:4px;overflow-x:auto;margin-bottom:9px;padding-bottom:4px">${components.map(component => `<span style="flex:0 0 auto;border-bottom:4px solid ${clusterColor(component.category)};background:${C.card2};padding:6px 7px;font-size:10px;color:${C.text}">${esc(component.text)}</span>`).join('')}</div><div style="display:flex;gap:8px;overflow-x:auto;overscroll-behavior-x:contain;padding-bottom:7px">${components.map(component => { const response = component.forwardResponse || {}, hold = component.hookHoldContribution || {}, direct = component.wholeHookOutcomeContributions || {}, holdValue = numeric(hold.effectHoldZ); return `<div style="flex:0 0 300px;border-left:3px solid ${clusterColor(component.category)};padding:8px 10px;background:${C.card2}"><div style="font-size:8px;color:${clusterColor(component.category)};font-weight:900">COMPONENT ${component.index + 1} · CLUSTER ${component.category}</div><div style="font-size:10px;color:${C.text};font-weight:800;line-height:1.4;margin:3px 0">${esc(component.text)}</div><div style="font-size:7px;color:${C.mute};line-height:1.45">cut evidence L ${component.leftBoundaryProbability == null ? 'hook edge' : fmt(component.leftBoundaryProbability, 3)} · R ${component.rightBoundaryProbability == null ? 'hook edge' : fmt(component.rightBoundaryProbability, 3)} · category confidence ${fmt(Number(component.categoryProbability || 0) * 100, 1)}%</div><div style="display:flex;justify-content:space-between;gap:8px;align-items:flex-end;margin:7px 0 5px"><span style="font-size:8px;color:${C.mute}">Hook Hold deletion effect<br><span style="font-size:7px">higher = predicted hook worsens without it</span></span><b style="font-size:20px;color:${holdValue >= 0 ? C.green : C.red}">${signed(holdValue, 2)}σ</b></div><div style="font-size:8px;color:${C.dim};line-height:1.55">${fmt(hold.categoryPercentile, 1)}th among training C${component.category} component effects · fixed-duration endpoint ${signed(hold.fixedDurationEndpointEffectPercentagePoints, 2)} pp${hold.counterfactualLeavesText ? '' : '<br>deletion leaves no text; comparison uses the fitted model intercept'}</div><div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:3px;margin-top:6px">${hookOutcomeOrder.map(target => `<span style="font-size:7px;color:${C.mute};border-left:2px solid ${C.border};padding-left:4px">${esc((((outcomes.targets || {})[target] || {}).shortLabel) || target)}<br><b style="color:${C.text}">${formatHookOutcomeEffect((direct[target] || {}).effect, target)}</b></span>`).join('')}</div><div style="height:1px;background:${C.border};margin:7px 0"></div><div style="font-size:7.8px;color:${C.dim};line-height:1.55">conditional response ${fmt(response.percentile, 1)}th · coordinate ${signed(response.axisCoordinate, 4)} · category rho ${fmt(response.heldoutSpearmanForCategory, 3)}<br>retained-information deletion ${signed(component.retainedInformationDeletionEffect, 4)} · ${fmt(component.categoryContributionPercentile, 1)}th · bootstrap ${signed(component.bootstrapP10, 4)} to ${signed(component.bootstrapP90, 4)}</div></div>`; }).join('')}</div>`, 'margin-bottom:10px')}
            <div class="pl-split" style="display:grid;grid-template-columns:minmax(280px,.8fr) minmax(0,1.2fr);gap:10px;margin-bottom:10px">${card(`<div style="font-size:11px;color:${C.text};font-weight:900;margin-bottom:5px">${pairs.length} local relationship measurements</div><div style="font-size:8px;color:${C.cyan};font-weight:900;line-height:1.5;margin-bottom:5px">HOOK HOLD INTERACTION · SAME METRIC AS HEADLINE</div>${holdInteractionTable}<div style="font-size:8px;color:${C.mute};line-height:1.5;margin-top:6px">Each populated cell is score(full) − score(without left) − score(without right) + score(without both), displayed in Hook Hold σ. Positive means the pair works better together than their separate deletion effects imply; negative means redundancy or interference inside this fitted model.</div><div style="height:1px;background:${C.border};margin:9px 0"></div><div style="font-size:8px;color:${C.mute};line-height:1.5;margin-bottom:5px">Conditional forward-response interaction · separate diagnostic</div>${forwardInteractionTable}<div style="font-size:8px;color:${C.mute};line-height:1.5;margin-top:6px">This matrix uses the later component's category-specific ${fmt(forwardMetric.selectedLagSeconds, 1)}s response coordinate. It is not blended into Hook Hold.</div><div style="height:1px;background:${C.border};margin:9px 0"></div><div style="font-size:9px;color:${C.text};font-weight:900;margin-bottom:4px">Broad retained-information local interaction</div>${interactionTable}`)}${card(`<div style="font-size:11px;color:${C.text};font-weight:900;margin-bottom:5px">Exact local counterfactual embedding inputs</div><div style="font-size:8px;color:${C.mute};line-height:1.5;margin-bottom:6px">One full-context deletion per component and one two-component deletion per pair. Source order and all retained source characters are preserved.</div><div style="max-height:460px;overflow:auto">${(local.componentDeletions || []).map(row => { const component = components[Number(row.removedComponent)] || {}, hold = component.hookHoldContribution || {}; return `<details style="border-top:1px solid ${C.border};padding:5px 0"><summary style="cursor:pointer;color:${C.dim};font-size:8.5px">remove component ${Number(row.removedComponent) + 1} · Hook Hold ${signed(hold.effectHoldZ, 2)}σ · retained-info ${signed(row.effect, 4)}</summary><div style="font-size:9px;color:${C.text};line-height:1.5;padding:5px 8px"><b>exact embedded text:</b> ${esc(row.embeddingInput || '(empty hook; fitted intercept)')}<br><b>Hook Hold:</b> ${signed(hold.fullRawCoordinate, 4)} full − ${signed(hold.withoutRawCoordinate, 4)} without = ${signed(hold.effectRawCarryPointsPerSecond, 4)} carry pp/s<br><b>fixed-duration endpoint effect:</b> ${signed(hold.fixedDurationEndpointEffectPercentagePoints, 3)} pp</div></details>`; }).join('')}${(local.pairDeletions || []).map(row => { const key = (row.removedComponents || []).join('-'), relation = pairLookup[key] || {}, hold = relation.hookHoldInteraction || {}; return `<details style="border-top:1px solid ${C.border};padding:5px 0"><summary style="cursor:pointer;color:${C.dim};font-size:8.5px">remove components ${(row.removedComponents || []).map(value => value + 1).join(' + ')} · Hook Hold interaction ${signed(hold.interactionHoldZ, 2)}σ</summary><div style="font-size:9px;color:${C.text};line-height:1.5;padding:5px 8px"><b>without pair:</b> ${esc(row.embeddingInput || '(empty hook; fitted intercept)')}<br><b>pair retained alone:</b> ${esc(row.retainedPairEmbeddingInput || '(empty)')}<br><b>interaction:</b> ${signed(hold.interactionRawCarryPointsPerSecond, 4)} carry pp/s · ${fmt(hold.categorySequencePercentile, 1)}th same-category-sequence percentile</div></details>`; }).join('')}</div>`)}</div>${componentScoreGallery(focus)}
            ${card(`<div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;flex-wrap:wrap"><div style="min-width:280px;flex:1"><div style="font-size:11px;color:${C.text};font-weight:900">Timing and scope decision</div><div style="font-size:9px;color:${C.dim};line-height:1.55;margin-top:4px">Dedicated component response lag: <b style="color:${forward.validatedAtComponentLevel ? C.green : C.red}">+${fmt(forwardMetric.selectedLagSeconds, 1)}s ${forward.validatedAtComponentLevel ? 'validated' : 'not validated'}</b>. Because that lag did not validate, the headline Hook Hold score uses the neutral spoken endpoint with zero added lag. Every component now has two distinct outputs: a same-metric Hook Hold deletion effect for full-score accounting, and the older category-specific unexpected-slope coordinate for timing research. They are shown separately and never silently averaged.</div></div><div style="font-size:8px;color:${C.mute};line-height:1.5">Boundary outcomes: none<br>Headline target: duration-adjusted carry lift<br>Component headline effect: exact local deletion<br>Relationship headline effect: exact second-order deletion<br>Conditional timing target: unexpected normalized slope<br>Examples in training: no<br>Causal claim: no</div></div><div style="height:1px;background:${C.border};margin:9px 0"></div><div style="font-size:9px;color:${C.text};font-weight:900;margin-bottom:4px">Nearest training hooks</div>${(result.nearestTrainingHooks || []).map(row => `<div style="display:flex;justify-content:space-between;gap:8px;border-top:1px solid ${C.border};padding:5px 0;font-size:8.5px"><span style="color:${C.dim}">${esc(row.text)}</span><b style="color:${C.text}">${fmt(row.cosine, 3)}</b></div>`).join('')}`)}</div>`;
        }
        ...
        function renderHookScorer() {
            ...
            return `${partitionMethodPanel(summary)}${card(`<div style="display:flex;gap:8px;align-items:flex-end;flex-wrap:wrap"><label style="flex:1;min-width:280px"><span style="display:flex;justify-content:space-between;gap:8px;font-size:8px;color:${C.mute};font-weight:900;text-transform:uppercase;margin-bottom:4px"><span>Complete hook or longer spoken opening</span><span data-pl-hook-score-count>${state.hookScoreText.length.toLocaleString()}/1,200 characters</span></span><textarea data-pl-hook-score-input maxlength="1200" rows="3" ${state.hookScoreLoading ? 'disabled' : ''} style="width:100%;box-sizing:border-box;resize:vertical;background:${C.card2};border:1px solid ${C.border};color:${C.text};padding:8px;font:10px/1.45 inherit;border-radius:6px;cursor:${state.hookScoreLoading ? 'wait' : 'text'}">${esc(state.hookScoreText)}</textarea><span style="display:block;font-size:7.5px;color:${C.mute};line-height:1.45;margin-top:3px">The exact scorer never truncates. Length support is reported against the measured 8–57-token training range; longer openings are computed but explicitly marked as extrapolations.</span></label><button data-pl-run-hook-score ${state.hookScoreLoading ? 'disabled' : ''} style="height:34px;border:1px solid ${C.cyan};background:${C.cyan}20;color:${C.cyan};padding:0 13px;border-radius:6px;font-size:10px;font-weight:900;cursor:${state.hookScoreLoading ? 'wait' : 'pointer'}">${state.hookScoreLoading ? 'Scoring…' : 'Score hook'}</button></div>${state.hookScoreError ? `<div data-pl-hook-score-error style="font-size:9px;color:${C.red};margin-top:7px">${esc(state.hookScoreError)}</div>` : ''}`, 'margin-bottom:10px')}
                <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px">${stat('training hooks', model.trainingHooks || 0, C.cyan)}${stat('random-fold rho', fmt(survival.heldoutSpearman, 3), C.text)}${stat('future rho', fmt((survival.chronologicalValidation || {}).heldoutSpearman, 3), C.red)}${stat('random permutation p', fmt((survival.rankInference || {}).p, 5), C.purple)}${stat('normalization robust', ((state.data.hookOutcomes.survivalModel || {}).normalizationSensitivity || {}).robustAcrossNormalizationChoices ? 'yes' : 'no', C.red)}${stat('generative LLM', 'NO', C.green)}</div>${survivalMethodPanel()}${predictionAccuracyPanel(activeHookOutcomeFocus())}
            ${card(`<div style="font-size:9px;color:${C.dim};line-height:1.55"><b style="color:${C.amber}">Why the headline remains provisional:</b> Hook Hold is now the single deterministic score used for the whole input, component deletion effects, and pair interactions. Its random-fold association is real, but it does not transfer to later videos and changes under a reasonable future-free normalization. It is therefore a complete model score, not yet a promoted universal or causal hook-quality truth.</div>`, 'margin-bottom:10px;border-color:' + C.amber + '55')}
        ...
        function drawComponentScoreAxis(canvas) {
            ...
            const observedFor = row => axis === 'hold'
            ...
            if (focus && focus.type === 'stored') {
                ...
                let point;
                if (axis === 'hold') point = [numeric((component.hookHoldContribution || {}).effectHoldZ), numeric(component.singletonAxisCoordinate)];
