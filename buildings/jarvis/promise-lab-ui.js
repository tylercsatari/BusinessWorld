(function () {
    ...
    window.createShortsPromiseLab = function createShortsPromiseLab(options) {
        ...
        function sealedBlindPanel(summary) {
            ...
            const sensitivity = audit.nearDuplicateThresholdSensitivity || [];
            const checks = [
                ...
                ['content isolation', blind.blindIsolationComputedBeforeOutcomeJoin === true, 'duplicate graph sealed before outcomes'],
            ...
            return `<div style="border-left:3px solid ${C.green};background:${C.card2};padding:8px;margin-top:8px"><div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;flex-wrap:wrap"><div><div style="font-size:9px;color:${C.green};font-weight:900">SEALED BLIND VALIDATION</div><div style="font-size:8px;color:${C.dim};line-height:1.5;margin-top:2px">Frozen predictions and every predeclared content-isolation policy were written and hashed before measured retention was opened. All source videos remain inspectable; repost members share one outcome-free content-component vote.</div></div><div style="font-size:7px;color:${C.mute};text-align:right;line-height:1.5">prediction manifest ${esc(String(blind.predictionManifestFingerprint || '').slice(0, 12))}<br>outcome manifest ${esc(String(blind.outcomeManifestFingerprint || '').slice(0, 12))}<br>blind generation ${esc(String(blind.blindGenerationId || '').slice(0, 12))}</div></div><div style="display:flex;gap:7px;flex-wrap:wrap;margin-top:7px">${stat('strict blind sources', Number(audit.strictBlindEligibleVideos || 0).toLocaleString(), C.green, 'retained source videos')}${stat('content components', Number(audit.strictBlindContentComponents || audit.strictBlindUniqueComponents || 0).toLocaleString(), C.cyan, 'independent statistical votes')}${stat('identity unverifiable', Number(audit.identityUnverifiableVideos || 0).toLocaleString(), C.amber, 'fewer than 3 normalized tokens')}${stat('training overlap', Number(audit.trainingContentOverlapExcluded || 0).toLocaleString(), C.red, `${Number(audit.exactTrainingContentOverlapExcluded || 0)} exact + ${Number(audit.nearTrainingContentOverlapExcluded || 0)} near excluded`)}${stat('repost members consolidated', Number(audit.externalDuplicateVideosCollapsed || 0).toLocaleString(), C.amber, 'visible rows, fractional component weight')}</div>${sensitivity.length ? `<div style="display:flex;gap:5px;flex-wrap:wrap;margin-top:7px">${sensitivity.map(row => `<div style="border:1px solid ${Number(row.threshold) === Number(audit.nearDuplicateThreshold) ? C.green : C.border};padding:4px 6px;font-size:7px;color:${C.dim}"><b style="color:${Number(row.threshold) === Number(audit.nearDuplicateThreshold) ? C.green : C.text}">Jaccard ≥ ${fmt(row.threshold, 2)}</b> · ${row.strictBlindEligibleVideos} sources / ${row.strictBlindComponents} components · ${row.nearTrainingOverlapVideos} near training</div>`).join('')}</div>` : ''}<div style="font-size:7px;color:${C.mute};line-height:1.45;margin-top:5px">${esc(audit.nearDuplicateDefinition || '')}</div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:5px;margin-top:7px">${checks.map(([label, pass, detail]) => `<div style="border-top:1px solid ${C.border};padding-top:4px;font-size:7.5px;color:${pass ? C.green : C.red}"><b>${pass ? 'PASS' : 'FAIL'} · ${esc(label)}</b><div style="color:${C.mute};margin-top:1px">${esc(detail)}</div></div>`).join('')}</div></div>`;
        }
        ...
        function accountValidationTable(summary) {
            ...
            return `<div style="background:${C.card2};padding:7px;margin-top:8px;overflow:auto"><div style="font-size:8px;color:${C.text};font-weight:900">ACCOUNT-BY-ACCOUNT BLIND TRANSPORT</div><div style="font-size:7px;color:${C.mute};line-height:1.45;margin-top:2px">Equal-account macro curve MAE ${fmt(balanced.macroSourceEqualCurveMAEPercentagePoints, 2)} pp · worst account ${fmt(balanced.worstAccountCurveMAEPercentagePoints, 2)} pp. Every account remains visible instead of being pooled away.</div><table style="width:100%;border-collapse:collapse;font-size:7.5px;margin-top:5px;min-width:820px"><thead><tr>${['account', 'source videos', 'content components', 'curve MAE', '20s MAE', '20s bias', '20s band coverage', '20s discrimination'].map(label => `<th style="padding:4px;text-align:right;color:${C.mute};border-bottom:1px solid ${C.border}">${label}</th>`).join('')}</tr></thead><tbody>${accounts.map(row => { const fixed = row.fixed20Second || {}; const discrimination = fixed.discriminationStatus === 'unavailable-constant-prediction' ? 'none · constant forecast' : (fixed.discriminationStatus || 'unavailable'); return `<tr><td style="padding:5px;text-align:left;color:${C.text};border-bottom:1px solid ${C.border}">${esc(names[String(row.accountId)] || row.accountId)}</td><td style="padding:5px;text-align:right;border-bottom:1px solid ${C.border}">${Number(row.videos || 0).toLocaleString()}</td><td style="padding:5px;text-align:right;border-bottom:1px solid ${C.border};color:${C.cyan}">${Number(row.contentComponents || row.videos || 0).toLocaleString()}</td><td style="padding:5px;text-align:right;border-bottom:1px solid ${C.border}">${fmt(row.sourceEqualCurveMAEPercentagePoints, 2)} pp<br><span style="font-size:6.5px;color:${C.mute}">${esc(confidenceText(row.sourceEqualCurveMAEConfidence95, 2))}</span></td><td style="padding:5px;text-align:right;border-bottom:1px solid ${C.border}">${fmt(fixed.maePercentagePoints, 2)} pp</td><td style="padding:5px;text-align:right;border-bottom:1px solid ${C.border};color:${Math.abs(Number(fixed.biasPercentagePoints || 0)) > 4 ? C.red : C.dim}">${signed(fixed.biasPercentagePoints, 2)} pp</td><td style="padding:5px;text-align:right;border-bottom:1px solid ${C.border}">${pct(Number(fixed.predictionBandCoverageFraction || 0) * 100)}</td><td style="padding:5px;text-align:right;border-bottom:1px solid ${C.border};color:${discrimination.startsWith('none') ? C.red : C.green}">${esc(discrimination)}</td></tr>`; }).join('')}</tbody></table></div>`;
        }

        function candidateDiagnostic(summary) {
            if (state.scope !== 'all') return '';
            const evaluation = summary.evaluation || {};
            const result = ((((evaluation.strictBlindCandidateVsBaseline || {}).families || {})[state.curveMode]) || {});
            if (!result.videos) return '';
            const fixedRows = Object.values(result.fixedHorizons || {}).filter(Boolean).sort((left, right) => Number(left.second) - Number(right.second));
            const leakage = (((evaluation.strictBlindCandidateLeakageSensitivity || {}).families || {})[state.curveMode] || {}).policies || [];
            const improvement = Number(result.pairedImprovementPercentagePoints || 0);
            const macro = result.equalAccountMacro || {};
            const fixed20 = result.fixed20Second || {};
            const confirmed = fixed20.blindSkillStatus === 'passes-error-and-ranking-gates';
            const verdict = confirmed ? 'candidate passes both predeclared blind gates at 20 seconds for review only' : 'candidate has no confirmed positive blind skill';
            return `<div style="border-left:3px solid ${confirmed ? C.amber : C.red};background:${C.card2};padding:7px;margin-top:8px"><div style="display:flex;justify-content:space-between;gap:8px;align-items:flex-start;flex-wrap:wrap"><div><div style="font-size:8px;color:${C.text};font-weight:900">BLIND CANDIDATE SKILL · DIAGNOSTIC ONLY</div><div style="font-size:7.5px;color:${C.dim};line-height:1.5;margin-top:3px">${Number(result.sourceVideos || result.videos || 0).toLocaleString()} transcript-backed source videos form ${Number(result.contentComponents || 0).toLocaleString()} content-component votes. Component-weighted improvement ${signed(improvement, 3)} pp (${esc(confidenceText(result.pairedImprovementConfidence95, 3))}); equal-account improvement ${signed(macro.pairedImprovementPercentagePoints, 3)} pp (${esc(confidenceText(macro.pairedImprovementConfidence95, 3))}). Candidate wins ${pct(Number(result.candidateWinFraction || 0) * 100)} of component-weighted repost members.</div></div><div style="font-size:7.5px;color:${confirmed ? C.amber : C.red};font-weight:900;max-width:430px;text-align:right">${esc(verdict)}. The deployed model stage remains unchanged.</div></div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:8px;margin-top:8px"><div style="background:${C.card};padding:7px;min-width:0"><div style="font-size:8px;color:${C.text};font-weight:900">EQUAL-ACCOUNT CANDIDATE IMPROVEMENT BY HORIZON</div><div style="font-size:7px;color:${C.mute};margin-top:2px">Above zero means lower error. Whiskers resample content components inside each account, then weight accounts equally.</div><canvas data-pl-canvas="pooled-candidate-horizon" style="display:block;width:100%;height:280px;margin-top:3px"></canvas></div><div style="background:${C.card};padding:7px;min-width:0"><div style="font-size:8px;color:${C.text};font-weight:900">NEAR-DUPLICATE POLICY SENSITIVITY</div><div style="font-size:7px;color:${C.mute};margin-top:2px">Every predeclared outcome-free cutoff is shown; none is selected because it looks best.</div><canvas data-pl-canvas="pooled-candidate-leakage" style="display:block;width:100%;height:280px;margin-top:3px"></canvas></div></div><div style="overflow:auto;margin-top:7px"><table style="width:100%;min-width:1120px;border-collapse:collapse;font-size:7.5px"><thead><tr>${['horizon', 'sources / components', 'component-weighted Δ', 'equal-account Δ', 'paired sign-flip Holm p', 'candidate spread', 'Pearson', 'rank permutation Holm p', 'blind result'].map(label => `<th style="padding:4px;text-align:right;color:${C.mute};border-bottom:1px solid ${C.border}">${label}</th>`).join('')}</tr></thead><tbody>${fixedRows.map(row => { const account = row.equalAccountMacro || {}; return `<tr><td style="padding:4px;text-align:right;border-bottom:1px solid ${C.border};color:${C.cyan}">${fmt(row.second, 0)}s</td><td style="padding:4px;text-align:right;border-bottom:1px solid ${C.border}">${Number(row.sourceVideos || row.videos || 0).toLocaleString()} / ${Number(row.contentComponents || 0).toLocaleString()}</td><td style="padding:4px;text-align:right;border-bottom:1px solid ${C.border};color:${Number(row.pairedImprovementPercentagePoints || 0) > 0 ? C.green : C.red}">${signed(row.pairedImprovementPercentagePoints, 3)} pp<br><span style="font-size:6.5px;color:${C.mute}">${esc(confidenceText(row.pairedImprovementConfidence95, 3))}</span></td><td style="padding:4px;text-align:right;border-bottom:1px solid ${C.border};color:${Number(account.pairedImprovementPercentagePoints || 0) > 0 ? C.green : C.red}">${signed(account.pairedImprovementPercentagePoints, 3)} pp<br><span style="font-size:6.5px;color:${C.mute}">${esc(confidenceText(account.pairedImprovementConfidence95, 3))}</span></td><td style="padding:4px;text-align:right;border-bottom:1px solid ${C.border}">${fmt(row.pairedImprovementSignFlipHolmAdjustedP, 3)}</td><td style="padding:4px;text-align:right;border-bottom:1px solid ${C.border}">${fmt(row.candidatePredictedStandardDeviationPercent, 2)} pp</td><td style="padding:4px;text-align:right;border-bottom:1px solid ${C.border}">${fmt(row.candidatePearson, 3)}</td><td style="padding:4px;text-align:right;border-bottom:1px solid ${C.border}">${fmt(row.accountStratifiedPositivePearsonPermutationHolmAdjustedP, 3)}</td><td style="padding:4px;text-align:right;border-bottom:1px solid ${C.border};color:${row.blindSkillStatus === 'passes-error-and-ranking-gates' ? C.green : C.red}">${esc(row.blindSkillStatus || 'unavailable')}</td></tr>`; }).join('')}</tbody></table></div>${leakage.length ? `<div style="font-size:7px;color:${C.mute};margin-top:5px">Isolation sensitivity: ${leakage.map(row => `${esc(row.label)} ${Number(row.strictBlindVideos || 0).toLocaleString()} sources / ${Number(row.strictBlindContentComponents || 0).toLocaleString()} components`).join(' · ')}.</div>` : ''}</div>`;
        }
        ...
        function evaluationPanel(summary, rows) {
            ...
            const primaryLabel = state.scope === 'all'
                ? 'strict external content components' : `${state.scope} frozen predictions`;
            ...
            return panel(`<div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;flex-wrap:wrap"><div><div style="font-size:12px;color:${C.text};font-weight:900">Frozen prediction accuracy against actual retention</div><div style="font-size:8px;color:${C.mute};line-height:1.55;margin-top:2px">Primary metrics use ${esc(primaryLabel)}. Time zero is excluded. Each sealed content component contributes one vote, regardless of repost count or duration. The fixed 20-second readout compares like with like; cross-horizon endpoint correlation is withheld.</div><div style="display:flex;gap:4px;margin-top:6px">${button('Normalized survival', 'data-pl-curve="entryIndexed"', state.curveMode === 'entryIndexed')}${button('Raw retention', 'data-pl-curve="observedAbsolute"', state.curveMode === 'observedAbsolute')}</div></div><div style="font-size:7.5px;color:${C.amber};max-width:510px;text-align:right;line-height:1.5">${esc((summary.evaluation || {}).claimBoundary || '')}</div></div>${sealedBlindPanel(summary)}<div style="display:flex;gap:9px;flex-wrap:wrap;margin-top:8px">${stat('evaluated source videos', Number(metrics.sourceVideos || metrics.videos || 0).toLocaleString(), C.text, state.scope === 'all' ? `${Number(pooled.videos || 0).toLocaleString()} visible pooled rows` : `${rows.length.toLocaleString()} visible rows`)}${stat('independent content votes', Number(metrics.contentComponents || metrics.videos || 0).toLocaleString(), C.cyan, 'repost members average into one component')}${stat('whole-curve MAE', `${fmt(metrics.sourceEqualCurveMAEPercentagePoints, 2)} pp`, C.cyan, `${confidenceText(metrics.sourceEqualCurveMAEConfidence95, 2)} · component bootstrap`)}${stat('20s MAE', `${fmt(fixed20.maePercentagePoints, 2)} pp`, C.amber, `${Number(fixed20.contentComponents || fixed20.videos || 0).toLocaleString()} components · bias ${signed(fixed20.biasPercentagePoints, 2)} pp`)}${stat('20s discrimination', noDiscrimination ? 'none' : fmt(fixed20.pearson, 3), noDiscrimination ? C.red : C.purple, noDiscrimination ? `constant forecast · predicted SD ${fmt(fixed20.predictedStandardDeviationPercent, 2)} vs actual ${fmt(fixed20.actualStandardDeviationPercent, 2)} pp` : `Pearson ${fmt(fixed20.pearson, 3)} · Spearman ${fmt(fixed20.spearman, 3)}`)}${stat('20s band coverage', pct(Number(fixed20.predictionBandCoverageFraction || 0) * 100), C.green, coverageSub)}</div>${state.scope === 'all' ? `<div style="font-size:7.5px;color:${C.dim};margin-top:7px">Transcript-backed MAE ${fmt(transcriptMetrics.sourceEqualCurveMAEPercentagePoints, 2)} pp across ${Number(transcriptMetrics.sourceVideos || transcriptMetrics.videos || 0).toLocaleString()} sources / ${Number(transcriptMetrics.contentComponents || 0).toLocaleString()} components. Identity-unverifiable rows are reported separately and never enter the strict headline.</div>` : ''}${accountValidationTable(summary)}${candidateDiagnostic(summary)}<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:8px;margin-top:9px"><div style="background:${C.card2};padding:7px"><div style="font-size:8px;color:${C.text};font-weight:900">COMPONENT-MEAN PREDICTED VS ACTUAL CURVE</div><canvas data-pl-canvas="pooled-mean" style="display:block;width:100%;height:300px;margin-top:3px"></canvas></div><div style="background:${C.card2};padding:7px"><div style="font-size:8px;color:${C.text};font-weight:900">COMPONENT-WEIGHTED ERROR BY SECOND</div><canvas data-pl-canvas="pooled-accuracy" style="display:block;width:100%;height:300px;margin-top:3px"></canvas></div></div><div style="background:${C.card2};padding:7px;margin-top:8px"><div style="font-size:8px;color:${C.text};font-weight:900">FIXED 20-SECOND RETENTION · PREDICTED VS ACTUAL</div><div style="font-size:7px;color:${noDiscrimination ? C.red : C.mute};margin-top:2px">${noDiscrimination ? 'Every content component receives the same frozen baseline at 20 seconds. This plot measures calibration spread, not ranking ability.' : 'Each dot is one strict blind content component at the same horizon.'}</div><canvas data-pl-canvas="pooled-scatter" style="display:block;width:100%;height:360px;margin-top:3px"></canvas></div>`, 'margin-top:9px');
        }
        ...
        function drawCandidateImprovement(canvas, rows, labelFor) {
            if (!rows.length) return;
            const sized = clearCanvas(canvas); const ctx = sized.context;
            const values = [0];
            rows.forEach(row => {
                values.push(Number(row.pairedImprovementPercentagePoints || 0));
                const interval = row.pairedImprovementConfidence95 || {};
                if (numeric(interval.lower) != null) values.push(Number(interval.lower));
                if (numeric(interval.upper) != null) values.push(Number(interval.upper));
            });
            const range = Math.max(0.2, Math.max(...values) - Math.min(...values));
            const yMin = Math.floor((Math.min(...values) - range * .18) * 10) / 10;
            const yMax = Math.ceil((Math.max(...values) + range * .18) * 10) / 10;
            const axes = drawAxes(ctx, sized.width, sized.height, '', 'candidate MAE improvement · pp', -.5, rows.length - .5, yMin, yMax);
            line(ctx, [[axes.X(-.5), axes.Y(0)], [axes.X(rows.length - .5), axes.Y(0)]], C.faint, 1.5, [4, 4]);
            rows.forEach((row, index) => {
                const value = Number(row.pairedImprovementPercentagePoints || 0);
                const interval = row.pairedImprovementConfidence95 || {};
                const lower = numeric(interval.lower) == null ? value : Number(interval.lower);
                const upper = numeric(interval.upper) == null ? value : Number(interval.upper);
                const x = axes.X(index); const color = value > 0 ? C.green : C.red;
                ctx.strokeStyle = color; ctx.lineWidth = 2;
                ctx.beginPath(); ctx.moveTo(x, axes.Y(lower)); ctx.lineTo(x, axes.Y(upper)); ctx.stroke();
                ctx.beginPath(); ctx.moveTo(x - 5, axes.Y(lower)); ctx.lineTo(x + 5, axes.Y(lower)); ctx.moveTo(x - 5, axes.Y(upper)); ctx.lineTo(x + 5, axes.Y(upper)); ctx.stroke();
                ctx.fillStyle = color; ctx.beginPath(); ctx.arc(x, axes.Y(value), 4, 0, Math.PI * 2); ctx.fill();
                ctx.fillStyle = C.text; ctx.font = '7px sans-serif';
                const label = labelFor(row); ctx.fillText(label, Math.max(axes.pad.l, x - Math.min(42, label.length * 2.1)), sized.height - 21);
            });
        }

        function drawPooledCandidateHorizon(canvas) {
            const result = (((((openingSummary() || {}).evaluation || {}).strictBlindCandidateVsBaseline || {}).families || {})[state.curveMode]) || {};
            const rows = Object.values(result.fixedHorizons || {}).filter(Boolean).sort((left, right) => Number(left.second) - Number(right.second)).map(row => ({
                ...row,
                pairedImprovementPercentagePoints: (row.equalAccountMacro || {}).pairedImprovementPercentagePoints,
                pairedImprovementConfidence95: (row.equalAccountMacro || {}).pairedImprovementConfidence95,
            }));
            drawCandidateImprovement(canvas, rows, row => `${fmt(row.second, 0)}s · c${row.contentComponents}`);
            canvas._plCandidateHorizonRows = rows.length;
        }

        function drawPooledCandidateLeakage(canvas) {
            const result = (((((openingSummary() || {}).evaluation || {}).strictBlindCandidateLeakageSensitivity || {}).families || {})[state.curveMode]) || {};
            const rows = (result.policies || []).map(row => ({
                ...row,
                pairedImprovementPercentagePoints: (row.equalAccountMacro || {}).pairedImprovementPercentagePoints,
                pairedImprovementConfidence95: (row.equalAccountMacro || {}).pairedImprovementConfidence95,
            }));
            drawCandidateImprovement(canvas, rows, row => `${row.label} · c${row.candidateContentComponents}`);
            canvas._plCandidateLeakageRows = rows.length;
        }

        function drawPooledScatter(canvas) {
            const summary = openingSummary() || {};
            const sourceRows = scopedLibraryRows(summary)
                ...
                return {
                    predicted, actual,
                    accountId: row.accountId || 'tyler',
                    componentId: row.blindContentComponentId || `video:${row.videoId}`,
                };
            }).filter(row => row.predicted != null && row.actual != null);
            const groups = new Map();
            sourceRows.forEach(row => {
                const group = groups.get(row.componentId) || [];
                group.push(row); groups.set(row.componentId, group);
            });
            const rows = [...groups.values()].map(group => ({
                predicted: group.reduce((sum, row) => sum + row.predicted, 0) / group.length,
                actual: group.reduce((sum, row) => sum + row.actual, 0) / group.length,
                accountId: group.map(row => row.accountId).sort()[0],
            }));
            if (!rows.length) return;
            const sized = clearCanvas(canvas); const ctx = sized.context;
        ...
        function drawAll() {
            ...
            host.querySelectorAll('canvas[data-pl-canvas]').forEach(canvas => {
                ...
                else if (kind === 'pooled-candidate-horizon') drawPooledCandidateHorizon(canvas);
                else if (kind === 'pooled-candidate-leakage') drawPooledCandidateLeakage(canvas);
