(function () {
    ...
    window.createLongQuantPromiseLab = function createLongQuantPromiseLab(deps) {
        ...
        function hookOutcomeAxisGallery(focus) {
            ...
            const axes = [
                { id: 'survival', label: 'Length-adjusted survival', definition: 'validated excess per-second carry after endpoint normalization and duration correction' },
        ...
        function survivalMethodPanel() {
            ...
            return card(`<div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap;margin-bottom:8px"><div style="min-width:300px;flex:1"><div style="font-size:9px;color:${C.cyan};font-weight:900;text-transform:uppercase">Verified score contract · all ${scope.videos || 208} videos rerun</div><div style="font-size:16px;color:${C.text};font-weight:900;margin-top:2px">Observed curve → additive replay envelope → duration-neutral carry → semantic score</div><div style="font-size:9px;color:${C.dim};line-height:1.6;margin-top:4px">For each measured video: <b style="color:${C.text}">C(t) = max(R(0)-100, 0) × clip((R(t)-F)/(R(0)-F), 0, 1)</b>, then <b style="color:${C.text}">R normalized = R observed - C</b>. F is the mean of the final max(3, 5%) measured samples. There is no fitted time-decay kernel. The graph below is additive correction in percentage points, not a retention curve.</div></div><div style="display:flex;gap:8px;flex-wrap:wrap">${stat('score rho', fmt(validation.heldoutSpearman, 3), C.green)}${stat('score p', fmt((validation.rankInference || {}).p, 5), C.purple)}${stat('entry model R²', fmt(terminalModel.oofR2, 3), C.cyan)}${stat('entry MAE', `${fmt(terminalModel.oofMAEPercentagePoints, 2)} pp`, C.text)}${stat('invented rises', geometry.correctionInducedIncreaseIntervals ?? '-', geometry.correctionInducedIncreaseIntervals === 0 ? C.green : C.red)}</div></div><div class="pl-split" style="display:grid;grid-template-columns:minmax(0,1.15fr) minmax(300px,.85fr);gap:10px"><div><canvas data-pl-canvas="replay-correction" style="width:100%;height:260px;display:block"></canvas><div style="font-size:8px;color:${C.mute};margin-top:4px">X = video second. Y = additive percentage points subtracted. Line = median across 208 measured curves; band = 10th–90th percentile across videos. It can remain above zero at 20s because every source is ${fmt(scope.minimumVideoDurationSeconds, 0)}s or longer and the anchor is near the full-video endpoint.</div></div><div style="font-size:8.5px;color:${C.dim};line-height:1.65"><b style="color:${C.text}">Observed support</b><br>Terminal retention ↔ start inflation: rho <b style="color:${C.green}">${fmt(entry.spearman, 3)}</b>, p ${fmt(entry.spearmanP, 4)}.<br>Terminal + duration predict entry out of fold with R² <b style="color:${C.green}">${fmt(terminalModel.oofR2, 3)}</b> and MAE ${fmt(terminalModel.oofMAEPercentagePoints, 2)} pp.<br><br><b style="color:${C.text}">Geometry checks</b><br>Start error ${fmt(geometry.maximumStartErrorPercentagePoints, 6)} pp · negative corrections ${geometry.negativeCorrectionValues ?? '-'} · correction-induced rises ${geometry.correctionInducedIncreaseIntervals ?? '-'} · max correction at the measured full-video endpoint ${fmt(geometry.maximumFullVideoEndpointCorrectionPercentagePoints, 4)} pp.<br>Median correction still present at 20s: ${fmt(geometry.medianCorrectionAt20SecondsPercentagePoints, 2)} pp; that is expected, not forced to zero.<br><br><b style="color:${C.text}">Claim boundary</b><br>${esc(audit.claimBoundary || '')}</div></div>`, 'margin-bottom:10px;border-color:' + C.cyan + '55');
        }
