(function () {
    ...
    window.createLongQuantPromiseLab = function createLongQuantPromiseLab(deps) {
        ...
        function forwardResponsePanel(summary) {
            ...
            ${card(`<div style="display:flex;justify-content:space-between;gap:14px;align-items:flex-start;flex-wrap:wrap"><div style="min-width:300px;flex:1"><div style="font-size:9px;color:${response.validated ? C.green : C.red};font-weight:900;text-transform:uppercase">Forward response metric · ${response.validated ? 'validated' : 'not validated'}</div><div style="font-size:17px;color:${C.text};font-weight:900;margin-top:3px">The measured response begins ${fmt(contract.selectedLagSeconds, 1)}s after each spoken instant</div><div style="font-size:9px;color:${C.dim};line-height:1.55;margin-top:5px">For every exact component interval [start, end], the selected ruler measures [start + ${fmt(contract.selectedLagSeconds, 1)}s, end + ${fmt(contract.selectedLagSeconds, 1)}s]. Outcome = endpoint-normalized retention slope minus the source-held-out text-free natural-drop expectation. Higher means flatter loss or a rise beyond expectation.</div></div><div style="display:flex;gap:7px;flex-wrap:wrap">${stat('selected lag', `+${fmt(contract.selectedLagSeconds, 1)}s`, C.cyan)}${stat('component rho', fmt(component.heldoutCategoryBalancedSpearman, 3), C.green)}${stat('nested p', fmt(inference.p, 4), C.purple)}${stat('timed components', `${((response.timingAudit || {}).componentsWithExactPositiveDuration || 0)}/${(response.audit || {}).components || 0}`, C.text)}</div></div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:8px;margin-top:9px;font-size:8.5px;line-height:1.5"><div style="border-left:3px solid ${C.cyan};padding-left:8px;color:${C.dim}"><b style="color:${C.text}">Lag evidence</b><br>Bootstrap median ${fmt(lag.medianLagSeconds, 1)}s · 10–90% ${fmt(lag.p10LagSeconds, 1)}–${fmt(lag.p90LagSeconds, 1)}s · +1s wins ${pct(Number(lag.selectedLagFraction || 0) * 100)} of source bootstraps.</div><div style="border-left:3px solid ${C.green};padding-left:8px;color:${C.dim}"><b style="color:${C.text}">All four categories replicate</b><br>${Object.entries(component.heldoutSpearmanByCategory || {}).map(([key, value]) => `C${key} ${fmt(value, 3)}`).join(' · ')}</div><div style="border-left:3px solid ${C.amber};padding-left:8px;color:${C.dim}"><b style="color:${C.text}">Scope boundary</b><br>Whole-hook component average p ${fmt(direct.p, 3)} and is rejected. The standalone relationship residual p ${fmt(relation.p, 3)} is predictive but remains a separate descriptive axis. The headline survival score is fitted directly from the complete hook; component axes are not averaged into it.</div></div>`, 'margin-bottom:10px;border-color:' + (response.validated ? C.green : C.red) + '66')}
        ...
        function hookOutcomeAxisGallery(focus) {
            ...
            const axes = [
                { id: 'survival', label: 'Length-adjusted survival', definition: 'validated excess per-second carry after rewatch and duration correction' },
                { id: 'quality', label: 'Retained information', definition: 'validated broad complete-hook coordinate; preserved as a separate diagnostic' },
        ...
        function retentionWordTable(forecast) {
            ...
            const adjusted = state.retentionCurveMode === 'adjusted';
        ...
        function retentionForecastPanel(forecast, stored = false) {
            ...
            return `<section style="border-top:1px solid ${C.border};border-bottom:1px solid ${C.border};padding:12px 0;margin:12px 0"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap"><div style="min-width:280px;flex:1"><div style="font-size:12px;color:${C.text};font-weight:900">${adjusted ? 'Rewatch-adjusted retention' : 'Observed absolute retention'} by second and by word</div><div style="font-size:8.5px;color:${C.dim};line-height:1.5;margin-top:3px">${stored ? 'Cyan is the source-held-out forecast; green is the measured curve.' : 'Cyan is the frozen-model forecast; the shaded band is the empirical 80% interval.'} Word response points use ${esc(forecast.wordTimingPolicy || 'library-average speaking rate')} and the measured +${fmt(forecast.responseLagSeconds == null ? 1 : forecast.responseLagSeconds, 1)}s response lag.</div><div style="display:flex;gap:5px;flex-wrap:wrap;margin-top:7px">${button('Rewatch-adjusted · starts 100', 'data-pl-retention-mode="adjusted"', adjusted)}${button('Observed absolute', 'data-pl-retention-mode="absolute"', !adjusted)}</div></div><div style="display:flex;gap:8px;flex-wrap:wrap">${stat('curve MAE', `${fmt(validation.heldoutMAEPercentagePoints, 2)} pp`, C.cyan)}${stat('baseline MAE', `${fmt(validation.baselineMAEPercentagePoints, 2)} pp`, C.dim)}${stat('improvement', pct(Number(validation.maeImprovementFraction || 0) * 100), C.green)}${stat('timewise rho', fmt(validation.meanTimewiseSpearman, 3), C.purple)}${stat('average speech', `${fmt(speaking.meanWordsPerSecond, 2)} words/s`, C.amber)}</div></div><canvas data-pl-canvas="retention-forecast" style="width:100%;height:360px;display:block;margin-top:7px"></canvas><div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:7px;margin-top:6px;font-size:8px;line-height:1.5"><div style="border-left:2px solid ${C.green};padding-left:7px;color:${C.dim}"><b style="color:${C.text}">WORDS SPOKEN</b><br>0 → ${fmt(forecast.spokenHookEndSeconds, 2)}s. Every visible word has exactly one component.</div><div style="border-left:2px solid ${C.cyan};padding-left:7px;color:${C.dim}"><b style="color:${C.text}">MEASURED RESPONSE</b><br>Extends through ${fmt(responseEnd, 2)}s because the selected response lag is forward-only.</div><div style="border-left:2px solid ${C.amber};padding-left:7px;color:${C.dim}"><b style="color:${C.text}">POST-HOOK FORECAST</b><br>${fmt(responseEnd, 2)} → ${fmt(forecastEnd, 0)}s uses the complete-hook embedding only. No unseen words or categories are invented.</div></div><div style="font-size:8px;color:${C.mute};margin-top:6px">Band coverage ${pct(Number(validation.empiricalBandCoverage || 0) * 100)} · paired improvement p ${fmt((validation.pairedImprovementInference || {}).p, 4)} · 41 measured-grid points from 0 to 20 seconds. All 208 source videos are at least 22 seconds; this is interpolation plus a held-out text forecast, never extrapolation beyond a source video. It remains observational, not causal.</div>${retentionWordTable(forecast)}</section>`;
        }

        function survivalScoreCard(score, stored = false) {
            if (!score) return '';
            return card(`<div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap"><div style="min-width:280px;flex:1"><div style="font-size:8px;color:${C.green};font-weight:900;text-transform:uppercase">Primary better/worse hook indicator</div><div style="font-size:18px;color:${C.text};font-weight:900;margin-top:2px">${fmt(score.percentile, 1)}th · length-adjusted survival</div><div style="font-size:9px;color:${C.dim};line-height:1.55;margin-top:4px">Higher means the hook loses less rewatch-adjusted retention than the ordinary loss for the same response duration. ${stored ? `This is a source-held-out prediction; the observed lift ranks ${fmt(score.actualPercentile, 1)}th.` : 'This is the frozen semantic prediction positioned among the 208 training-hook predictions.'}</div></div><div style="display:flex;gap:8px;flex-wrap:wrap">${stat('predicted carry / sec', `${fmt(score.predictedCarryPercentPerSecond, 3)}%`, C.green)}${stored ? stat('actual carry / sec', `${fmt(score.actualCarryPercentPerSecond, 3)}%`, C.cyan) : ''}${stat('ordinary carry / sec', `${fmt(score.expectedCarryPercentPerSecond, 3)}%`, C.dim)}${stat('response end', `${fmt(score.responseEndSeconds, 2)}s`, C.amber)}</div></div><div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin-top:9px;font-size:8.5px;line-height:1.5"><div style="border-left:2px solid ${C.cyan};padding-left:7px;color:${C.dim}"><b style="color:${C.text}">TARGET</b><br>Geometric percent retained per second through hook end + response lag.</div><div style="border-left:2px solid ${C.purple};padding-left:7px;color:${C.dim}"><b style="color:${C.text}">LENGTH CORRECTION</b><br>Subtract a fold-fitted baseline using response seconds, seconds², and log seconds.</div><div style="border-left:2px solid ${C.green};padding-left:7px;color:${C.dim}"><b style="color:${C.text}">PERCENTILE</b><br>Rank of the semantic model's predicted excess carry, not raw retention and not views.</div></div>`, 'margin-bottom:10px;border-color:' + C.green + '66');
        }

        function survivalMethodPanel() {
            const outcomes = state.data.hookOutcomes || {}, audit = outcomes.rewatchAudit || {};
            const validation = (outcomes.survivalModel || {}).validation || {};
            const scope = audit.scope || {}, entry = audit.entryInflationVsTerminal || {}, opening = audit.entryInflationVsOpeningThreeSecondSlope || {};
            return card(`<div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap;margin-bottom:8px"><div style="min-width:300px;flex:1"><div style="font-size:9px;color:${C.cyan};font-weight:900;text-transform:uppercase">Verified score contract · all ${scope.videos || 208} videos rerun</div><div style="font-size:16px;color:${C.text};font-weight:900;margin-top:2px">Observed curve → rewatch correction → duration-neutral carry → semantic score</div><div style="font-size:9px;color:${C.dim};line-height:1.6;margin-top:4px">The correction is empirical: subtract entry-above-100 multiplied by the measured decay kernel. It starts every curve at exactly 100 and reaches zero correction near 15 seconds, so the adjusted line converges back to the observed line. It is explicitly an index, not a causal claim of true first-pass retention.</div></div><div style="display:flex;gap:8px;flex-wrap:wrap">${stat('score rho', fmt(validation.heldoutSpearman, 3), C.green)}${stat('score p', fmt((validation.rankInference || {}).p, 5), C.purple)}${stat('MAE gain', pct(Number(validation.maeImprovementFraction || 0) * 100), C.cyan)}${stat('min video', `${fmt(scope.minimumVideoDurationSeconds, 0)}s`, C.text)}</div></div><div class="pl-split" style="display:grid;grid-template-columns:minmax(0,1.15fr) minmax(300px,.85fr);gap:10px"><div><canvas data-pl-canvas="rewatch-kernel" style="width:100%;height:260px;display:block"></canvas><div style="font-size:8px;color:${C.mute};margin-top:4px">X = video second. Y = share of entry inflation subtracted. Line = all-video fit; band = 10th–90th percentile across the five source-held-out folds.</div></div><div style="font-size:8.5px;color:${C.dim};line-height:1.65"><b style="color:${C.text}">Hypothesis test</b><br>Terminal retention ↔ start inflation: rho <b style="color:${C.green}">${fmt(entry.spearman, 3)}</b>, p ${fmt(entry.spearmanP, 4)}.<br>Start inflation ↔ 0–3s slope: rho <b style="color:${C.green}">${fmt(opening.spearman, 3)}</b>, p ${fmt(opening.spearmanP, 4)}.<br><br><b style="color:${C.text}">20-second integrity</b><br>${scope.videosShorterThan20Seconds || 0} sources end before 20s. Spoken hooks end at median ${fmt(scope.medianSpokenHookEndSeconds, 2)}s and maximum ${fmt(scope.maximumSpokenHookEndSeconds, 2)}s. Beyond the +1s response boundary, only the whole-hook continuation model is shown.<br><br><b style="color:${C.text}">Component percentile</b><br>Each +1s component number ranks that exact component's predicted unexpected endpoint-normalized slope among training components in the same frozen cluster. Higher = flatter/rising retention beyond the text-free natural-drop expectation.</div></div>`, 'margin-bottom:10px;border-color:' + C.cyan + '55');
        }
        ...
        function componentMetricValue(component, axis, focusType) {
            ...
            if (axis === 'broad') {
                ...
                return { percentile: focusType === 'live' ? row.categoryContributionPercentile : row.percentile, coordinate: focusType === 'live' ? row.retainedInformationDeletionEffect : row.deletionEffect, status: 'validated' };
            }
            if (axis === 'forward') {
        ...
        function componentScoreGallery(focus) {
            ...
            const targets = (state.data.hookOutcomes || {}).targets || {};
            const axes = [
                { id: 'broad', label: 'Broad retained-information deletion effect', note: 'X = full-hook axis minus the axis after deleting only this component; Y = standalone semantic coordinate.' },
        ...
        function renderHookScorer() {
            ...
            <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px">${stat('training hooks', model.trainingHooks || 0, C.cyan)}${stat('survival rho', fmt(survival.heldoutSpearman, 3), C.green)}${stat('survival p', fmt((survival.rankInference || {}).p, 5), C.purple)}${stat('survival MAE gain', pct(Number(survival.maeImprovementFraction || 0) * 100), C.text)}${stat('fold cosine', fmt(survival.foldDirectionMedianCosine, 3), C.amber)}${stat('generative LLM', 'NO', C.green)}</div>${survivalMethodPanel()}
            ${card(`<div style="font-size:9px;color:${C.dim};line-height:1.55"><b style="color:${C.amber}">Why this is not labeled virality:</b> the primary number now answers one narrower question: whether wording predicts less retention loss than normal for the hook's duration. It does not combine swipe rate, views, or the rest of the video into a causal virality score. The earlier retained-information axis remains visible as its own embedding plane and component attribution channel.</div>`, 'margin-bottom:10px;border-color:' + C.amber + '55')}
        ...
        function drawRewatchKernel(canvas) {
            ...
            drawLatencyLines(canvas, times, [{
                label: 'entry-inflation correction', color: C.cyan,
                values: (normalization.kernel || []).map(value => numeric(value) * 100),
                low: (audit.foldKernelP10 || []).map(value => numeric(value) * 100),
                high: (audit.foldKernelP90 || []).map(value => numeric(value) * 100),
            }], { xFormat: value => `${fmt(value, 0)}s`, yFormat: value => `${fmt(value, 0)}%` });
        }
        ...
        function drawRetentionForecast(canvas, mini = false) {
            ...
            if (!mini) {
                ...
                context.fillStyle = C.cyan; context.font = '8px sans-serif'; context.fillText(adjusted ? 'predicted adjusted' : 'predicted absolute', left, 10);
        ...
        function drawCanvases() {
            document.querySelectorAll('#pl-root canvas[data-pl-canvas]').forEach(canvas => {
                const kind = canvas.dataset.plCanvas;
                if (kind === 'rewatch-kernel') return drawRewatchKernel(canvas);
