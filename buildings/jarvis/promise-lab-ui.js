(function () {
    ...
    window.createLongQuantPromiseLab = function createLongQuantPromiseLab(deps) {
        ...
        function drawCurveAccuracy(canvas) {
            ...
            const times = (validation.timesSeconds || []).map(numeric);
            ...
            if (!times.length || times.length !== model.length || times.length !== baseline.length) return;
            const { context, width, height } = canvasContext(canvas);
            const pad = { left: 31, right: 10, top: 12, bottom: 22 };
            const maxY = Math.max(1, ...model, ...baseline) * 1.08;
            const x = value => pad.left + value / Math.max(...times, 1) * (width - pad.left - pad.right);
            ...
            const line = (values, color, lineWidth) => {
                ...
                values.forEach((value, index) => index ? context.lineTo(x(times[index]), y(value)) : context.moveTo(x(times[index]), y(value)));
                context.stroke();
            };
            line(baseline, C.mute, 1.2); line(model, C.cyan, 2.2);
            context.fillStyle = C.mute; context.font = '7px sans-serif';
            context.fillText('0s', pad.left, height - 7); context.textAlign = 'right'; context.fillText(`${fmt(Math.max(...times), 0)}s`, width - pad.right, height - 7); context.textAlign = 'left';
            ...
            canvas.dataset.plCurveAccuracyPoints = String(times.length);
        }
