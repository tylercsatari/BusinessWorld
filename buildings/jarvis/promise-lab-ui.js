(function () {
    ...
    window.createLongQuantPromiseLab = function createLongQuantPromiseLab(deps) {
        ...
        function survivalMethodPanel() {
            ...
            const promoted = String(validation.status || '').startsWith('validated');
            return card(`<div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap;margin-bottom:8px"><div style="min-width:300px;flex:1"><div style="font-size:9px;color:${promoted ? C.green : C.amber};font-weight:900;text-transform:uppercase">Hook Hold method audit · ${promoted ? 'robust' : 'diagnostic only'}</div><div style="font-size:16px;color:${C.text};font-weight:900;margin-top:2px">Observed curve → terminal-conditioned replay envelope → duration-neutral diagnostic</div><div style="font-size:9px;color:${C.dim};line-height:1.6;margin-top:4px">For each measured video: <b style="color:${C.text}">C(t) = max(R(0)-100, 0) × clip((R(t)-F)/(R(0)-F), 0, 1)</b>, then <b style="color:${C.text}">R normalized = R observed - C</b>. This geometry is deterministic, but F uses the end of the full video. That makes it an observational sensitivity index, not an identified replay correction available at hook time.</div></div><div style="display:flex;gap:8px;flex-wrap:wrap">${stat('random-fold rho', fmt(validation.heldoutSpearman, 3), C.text)}${stat('future-only rho', fmt(temporal.heldoutSpearman, 3), temporal.heldoutSpearman > 0 ? C.green : C.red)}${stat('entry-normalized rho', fmt(entryNormalized.heldoutSpearman, 3), C.amber)}${stat('robust to target', sensitivity.robustAcrossNormalizationChoices ? 'yes' : 'no', sensitivity.robustAcrossNormalizationChoices ? C.green : C.red)}${stat('invented rises', geometry.correctionInducedIncreaseIntervals ?? '-', geometry.correctionInducedIncreaseIntervals === 0 ? C.green : C.red)}</div></div><div style="border-left:3px solid ${promoted ? C.green : C.amber};padding:8px 10px;background:${promoted ? C.green : C.amber}0d;color:${C.dim};font-size:9px;line-height:1.6;margin-bottom:9px"><b style="color:${promoted ? C.green : C.amber}">${promoted ? 'PROMOTED' : 'HOOK HOLD IS NOT THE TRAINING REWARD'}</b><br>Random folds alone are not enough. Promotion requires positive future-only transfer and positive performance under the future-free entry-normalized target. The current artifact exposes all three checks and keeps this coordinate diagnostic; Market Hold is the separately validated training proxy shown above.</div><div class="pl-split" style="display:grid;grid-template-columns:minmax(0,1.15fr) minmax(300px,.85fr);gap:10px"><div><canvas data-pl-canvas="replay-correction" style="width:100%;height:260px;display:block"></canvas><div style="font-size:8px;color:${C.mute};margin-top:4px">X = normalized analyzed-hook progress from 0% to 100%. Y = additive percentage points subtracted. Line = source-equal median across ${scope.videos || 208} measured hooks; band = 10th–90th percentile. Source endpoints range ${fmt(scope.minimumResponseEndSeconds, 2)}–${fmt(scope.maximumResponseEndSeconds, 2)}s. This is the amount removed by one declared normalization, not a predicted retention curve.</div></div><div style="font-size:8.5px;color:${C.dim};line-height:1.65"><b style="color:${C.text}">Observed support</b><br>Terminal retention ↔ start inflation: rho <b style="color:${C.text}">${fmt(entry.spearman, 3)}</b>, p ${fmt(entry.spearmanP, 4)}. Association supports a replay hypothesis but does not identify replay counts.<br><br><b style="color:${C.text}">Geometry checks</b><br>Start error ${fmt(geometry.maximumStartErrorPercentagePoints, 6)} pp · negative corrections ${geometry.negativeCorrectionValues ?? '-'} · correction-induced rises ${geometry.correctionInducedIncreaseIntervals ?? '-'} · endpoint correction ${fmt(geometry.maximumFullVideoEndpointCorrectionPercentagePoints, 4)} pp.<br><br><b style="color:${C.text}">Sensitivity result</b><br>Terminal-conditioned versus entry-normalized prediction rho ${fmt((sensitivity.terminalVsEntryPrediction || {}).spearman, 3)}. Target rho ${fmt((sensitivity.terminalVsEntryTarget || {}).spearman, 3)}. These are displayed because a stable semantic direction should not reverse when a reasonable future-free normalization is substituted.</div></div>`, 'margin-bottom:10px;border-color:' + (promoted ? C.green : C.amber) + '55');
        }
