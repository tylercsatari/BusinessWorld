...
def gen_group(title, n=G, temp=1.05):
    """G reasoning+prompt attempts for ONE title. Thinking ON so we capture the reasoning trace."""
    text = tok.apply_chat_template([{"role": "system", "content": SYS}, {"role": "user", "content": title}],
                                   tokenize=False, add_generation_prompt=True, enable_thinking=True)
    ins = tok([text] * n, return_tensors="pt", padding=True).to("cuda")
    with torch.no_grad():
        out = model.generate(**ins, max_new_tokens=1500, do_sample=True, temperature=temp, top_p=0.95, pad_token_id=tok.pad_token_id)
    res = []
    for i in range(n):
        ...
        res.append({"reasoning": reasoning,
                    "prompt": pr if (isinstance(pr, str) and len(pr.strip()) > 8) else None,
                    "completion": full})
    return res
...
def process_input(item, iid, run=RUN, gkey=None):
...
for ii, item in enumerate(titles):
    iid = "t%05d" % ii
    ...
    try: process_input(item, iid)
    except H.BillingHalt as e: print("=== BILLING HALT: %s ===" % str(e)[:80], flush=True); break
