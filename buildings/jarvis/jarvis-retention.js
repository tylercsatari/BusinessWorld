...
const JarvisRetention = (function () {
    ...
    function renderRaw() {
        ...
        const PJ = R.proj || {};
        const PROJS = [['both', '→ views+outlier'], ['views', '→ views'], ['outlier', '→ outlier'], ['hi10m', '>10M class'], ['hiout', 'top-outlier'], ['umap', 'UMAP raw'], ['pca', 'PCA raw']].filter(p => PJ[p[0]]);
    ...
    async function mount(el) {
        ...
        if (!DATA && !err) {
            ...
            const loadJSON = async (url) => { const r = await fetch(url + (url.includes('?') ? '&' : '?') + 'v=76'); if (!r.ok) throw new Error('HTTP ' + r.status); const t = await r.text(); if (/^\s*</.test(t)) throw new Error('got HTML (deploy in progress)'); return JSON.parse(t); };
