...
const JarvisRetention = (function () {
    ...
    function savedChannelRiskCurve(signal, baseRate) {
        ...
        rows.forEach(row => { svg += `<circle cx="${X(row.threshold)}" cy="${Y(row.hitRate)}" r="3" fill="${C.cyan}"><title>score ≥ ${fv(row.threshold)} · ${row.hits}/${row.n} hit · 95% CI ${(row.ciLow * 100).toFixed(0)}–${(row.ciHigh * 100).toFixed(0)}%</title></circle>`; });
    ...
    function savedChannelRiskCalibration(signal) {
        ...
        bins.forEach(bin => {
            ...
            svg += `<line x1="${x}" y1="${Y(bin.ciLow)}" x2="${x}" y2="${Y(bin.ciHigh)}" stroke="${C.purple}" opacity=".7"/><circle cx="${x}" cy="${Y(bin.hitRate)}" r="${radius}" fill="${C.purple}" opacity=".78"><title>${bin.n} Shorts · score median ${fv(bin.scoreMedian)} · actual hit ${(bin.hitRate * 100).toFixed(0)}% · actual views median ${fv(bin.actualViewsMedian)}</title></circle>`;
        });
    ...
    function renderSavedChannelRisk(analysis) {
        ...
        const riskRow = row => {
            ...
            return `<tr style="border-top:1px solid ${C.border};background:${highlight ? C.cyan + '0b' : 'transparent'}"><td style="padding:6px;color:${highlight ? C.cyan : C.text};font-weight:${highlight ? 900 : 700}">≥ ${fv(row.threshold)}</td><td style="text-align:right">${row.n}</td><td style="text-align:right;color:${row.hitRate != null && row.hitRate >= (breakEven || 0) ? C.green : C.dim}">${row.hitRate == null ? '—' : (row.hitRate * 100).toFixed(0) + '%'}</td><td style="text-align:right">${row.ciLow == null ? '—' : `${(row.ciLow * 100).toFixed(0)}–${(row.ciHigh * 100).toFixed(0)}%`}</td><td style="text-align:right">${row.lift == null ? '—' : row.lift.toFixed(2) + '×'}</td><td style="text-align:right">${row.recall == null ? '—' : (row.recall * 100).toFixed(0) + '%'}</td><td style="text-align:right">${row.n ? (100 - row.hitRate * 100).toFixed(0) + '%' : '—'}</td><td style="text-align:right">${row.actualViewsP25 == null ? '—' : fv(row.actualViewsP25)}</td><td style="text-align:right">${row.actualViewsMedian == null ? '—' : fv(row.actualViewsMedian)}</td><td style="text-align:right;color:${expected != null && expected > 0 ? C.green : C.red}">${expected == null ? '—' : (expected >= 0 ? '+' : '') + expected.toFixed(2)}</td><td style="text-align:right;color:${conservative != null && conservative > 0 ? C.green : C.dim}">${conservative == null ? '—' : (conservative >= 0 ? '+' : '') + conservative.toFixed(2)}</td></tr>`;
        };
