...
const JarvisLongQuant = (function () {
    ...
    function rawEnsure(ch) { if (RAW[ch]) return; RAW[ch] = { loading: 1 }; fetch('/api/raw-long/map?channel=' + ch).then(r => r.json()).then(j => { RAW[ch] = j; rtgUpdateRaw(); }).catch(() => { RAW[ch] = { n: 0 }; rtgUpdateRaw(); }); }
    ...
    function renderRaw() {
        ...
        const accKey = key => (['ctr', 'ret30', 'realviews'].includes(key) && PJ[key + '__' + ACCT]) ? key + '__' + ACCT : key;
        const accHas = key => ['ctr', 'ret30', 'realviews'].includes(key) && !!PJ[key + '__' + ACCT];
        const PROJS = [['both', '→ views+outlier'], ['views', '→ views (log)'], ['rawviews', '→ views (raw)'], ['realviews', '→ realistic views'], ['outlier', '→ outlier'], ['hi10m', '>10M class'], ['hiout', 'top-outlier'], ['ctr', '→ CTR'], ['ret30', '→ 30s-retention'], ['umap', 'UMAP raw'], ['pca', 'PCA raw']].filter(p => PJ[accKey(p[0])]);
        let pm = st.rawProj || 'both'; if (!PJ[accKey(pm)]) pm = PROJS.length ? PROJS[0][0] : null;
        const proj = (pm && PJ[accKey(pm)]) || { x: R.x || [], y: R.y || [], cv: 0, co: 0 };
        const isAcctProj = ['ctr', 'ret30', 'realviews'].includes(pm);   // this projection is account-specific
        ...
        const projPill = ([id, lab]) => { const pj = PJ[accKey(id)], acc = ['ctr', 'ret30', 'realviews'].includes(id); return `<span data-rawproj="${id}" style="cursor:pointer;border:1px solid ${pm === id ? C.accent : C.border};background:${pm === id ? C.accent + '1e' : 'transparent'};color:${pm === id ? C.accent : C.dim};border-radius:6px;padding:3px 9px;font-size:10px;font-weight:700">${lab}${acc ? ` <span style="opacity:.55;font-weight:800;color:${C.green}">·${ACCT_NM}</span>` : ''}${pj && !['umap', 'pca'].includes(id) ? ` <span style="opacity:.65;font-weight:400">v${pj.cv}/o${pj.co}</span>` : ''}</span>`; };
