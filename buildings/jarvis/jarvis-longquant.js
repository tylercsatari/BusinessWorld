...
const JarvisLongQuant = (function () {
    ...
    function lgBands(pts, K, W, H, pad) {
        ...
        if (Math.hypot(a, b) < 1e-12) return { under: '', over: '', note: 'percentile is flat across this space (no gradient)' };
        ...
        for (let bi = 0; bi < K; bi++) { const bn = bins[bi]; if (bn.cnt < 2) continue; const cx = bn.gx / bn.cnt, cy = bn.gy / bn.cnt, s = bn.v.slice().sort((p, q) => p - q), txt = Math.round((s[s.length >> 1] || 0) * 100) + 'th', w = txt.length * 6.6 + 12; over += `<g style="pointer-events:none"><rect x="${(cx - w / 2).toFixed(1)}" y="${(cy - 9).toFixed(1)}" width="${w.toFixed(1)}" height="16" rx="4" fill="#0f172a" opacity="0.85" stroke="#1e293b"/><text x="${cx.toFixed(1)}" y="${(cy + 2.8).toFixed(1)}" text-anchor="middle" font-size="10" font-weight="700" fill="#e2e8f0">${txt}</text></g>`; }
        return { under, over, note: `the cyan <b>path</b> (→) is the direction percentile rises across this space; dashed lines cut <b>⟂</b> into ${K} equal-count bands, each labelled with its <b>median percentile</b>` };
    }
    ...
    function renderLongGuesses() {
        ...
        if (MANI && MANI.rows) {
            ...
            let bU = '', bO = '', bNote = '';
            if (st.lgBands) { const B = lgBands(rows.map(r => { const q = gpos(r); return { sx: X(q[0]), sy: Y(q[1]), v: r.pctile || 0 }; }), Math.max(2, Math.min(16, st.lgBandK || 6)), W, H, pad); bU = B.under; bO = B.over; bNote = B.note; }
