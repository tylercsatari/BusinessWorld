...
const JarvisLongQuant = (function () {
    ...
    // per-real-thumbnail metric for a latent space (the thing that space is organised by) + band-label formatter
    function lgProjMetric(RV, key) {
        const P = (RV && RV.proj && RV.proj[key]) || {}, views = (RV && RV.views) || [], nId = ((RV && RV.id) || []).length;
        if (P.est && P.est.length === nId) {   // ctr / ret30 / realviews store a per-thumbnail estimate
            const isViews = key === 'realviews';
            return { vals: P.est.map(x => isViews ? Math.log10((+x || 0) + 1) : +x),
                fmt: isViews ? (v => fv(Math.pow(10, v))) : (v => Math.round(v) + '%'),
                label: key === 'ctr' ? 'estimated CTR' : key === 'ret30' ? 'estimated 30s-retention' : key === 'realviews' ? 'estimated views' : 'estimated ' + key };
        }
        if (key === 'views') return { vals: views.map(v => Math.log10((+v || 0) + 1)), fmt: v => fv(Math.pow(10, v)), label: 'views' };
        if (key === 'hi10m') return { vals: views.map(v => (+v > 1e7 ? 1 : 0)), fmt: v => Math.round(v * 100) + '%', label: '>10M-view share' };
        if (key === 'outlier') { const o = (RV && RV.outlier) || []; if (o.length === nId) return { vals: o.map(x => Math.log10((+x || 0) + 1)), fmt: v => Math.pow(10, v).toFixed(1) + '×', label: 'outlier ratio' }; }
        return null;   // ctrviews / no per-thumbnail metric → band by the guess's own CTR+views percentile
    }
    ...
    function renderLongGuesses() {
        ...
        if (MANI && MANI.rows) {
            ...
            // the metric THIS space is organised by (est CTR / 30s-ret / views / >10M-share / outlier); ctrviews → the guess's own CTR+views percentile
            const M = proj === 'ctrviews' ? null : lgProjMetric(RV, proj);
            const gmetric = r => { if (!M) return r.pctile || 0; let s = 0, k = 0; (r.nbr || []).forEach(nb => { const i = id2i[String(nb[0])]; if (i != null && M.vals[i] != null && isFinite(M.vals[i])) { s += M.vals[i]; k++; } }); return k ? s / k : (r.pctile || 0); };
            const gm = rows.map(gmetric), gmOk = gm.filter(v => isFinite(v)), gmLo = gmOk.length ? Math.min.apply(null, gmOk) : 0, gmHi = gmOk.length ? Math.max.apply(null, gmOk) : 1;
            const metricLabel = M ? M.label : 'CTR+views percentile', bandFmt = M ? M.fmt : (v => Math.round((v || 0) * 100) + 'th');
            const colVal = i => M ? (gmHi > gmLo ? (gm[i] - gmLo) / (gmHi - gmLo) : 0.5) : (rows[i].pctile || 0);
            ...
            const dots = rows.map((r, i) => { const q = gpos(r), on = gsel && r.input_id === gsel, op = gsel ? (on ? 1 : 0.25) : 0.9; return `<circle data-lgpick="${r.input_id}" cx="${X(q[0]).toFixed(1)}" cy="${Y(q[1]).toFixed(1)}" r="${on ? 6.5 : 3.2}" fill="${lgHeat(colVal(i))}" opacity="${op}" stroke="${on ? '#fff' : 'rgba(0,0,0,.45)'}" stroke-width="${on ? 1.8 : 0.5}" style="cursor:pointer"><title>${esc((r.title || '').slice(0, 44))} · ${((r.pctile || 0) * 100).toFixed(0)}th ctrviews${M ? ' · ' + bandFmt(gm[i]) + ' ' + metricLabel : ''}</title></circle>`; }).join('');
            const projLbl = (projAvail.find(p => p[0] === proj) || [, proj])[1];
            let bU = '', bO = '', bNote = '';
            if (st.lgBands) { const B = lgBands(rows.map((r, i) => { const q = gpos(r); return { sx: X(q[0]), sy: Y(q[1]), v: gm[i] }; }), Math.max(2, Math.min(16, st.lgBandK || 6)), W, H, pad, bandFmt, metricLabel); bU = B.under; bO = B.over; bNote = B.note; }
        ...
        return cardc(`<div style="font-size:15px;font-weight:800;color:${C.text};margin-bottom:4px">🎰 Guesses — thumbnail RL</div><div style="font-size:11px;color:${C.mute};margin-bottom:8px">Every title the model tried + its 5 generated thumbnails. Click a title to light up its guesses on the map and see all five below. Switch the latent space to view the same guesses projected different ways.</div><div style="display:flex;gap:5px;flex-wrap:wrap;align-items:center;margin-bottom:5px"><span style="font-size:9px;color:${C.mute};text-transform:uppercase">round</span>${runPills || `<span style="font-size:11px;color:${C.dim}">no runs yet — starts when training kicks off</span>`}</div><div style="display:flex;gap:5px;flex-wrap:wrap;align-items:center;margin-bottom:5px"><span style="font-size:9px;color:${C.mute};text-transform:uppercase">latent space</span>${projPills}</div>${demo}${body}${detail}`, 14);
    }
