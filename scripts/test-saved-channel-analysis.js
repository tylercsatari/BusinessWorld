...
for (const marker of ['data-savedbank', 'data-savedchanneladd', 'data-savedchannelvideo', 'prediction analysis', "tabButton('visualization', 'Visualization')", "tabButton('ledger', 'Ledger')", 'data-savedvalidation-canonical', 'All ${scoreColumns.length} score interpretations × all ${outcomes.length} raw observed metrics', 'data-savedvalidation-ledger-navigator', 'data-independent-axis-picker', 'data-savedvalidation-coordinate-picker', 'data-savedvalidation-outcome-picker', 'data-savedvalidation-coordinate-order', 'data-savedvalidation-correlation-ranking', 'data-savedvalidation-ranked-coordinate', 'savedValidationRankedScoreColumns', 'Raw Spearman ρ from the selected scope', 'data-savedvalidation-open-ledger', 'data-savedledger-open-visualization', 'data-savedvalidation-view-tabs', 'data-savedvalidationview', 'data-savedvalidation-pane', 'data-savedvalidation-full-evidence', 'data-savedvalidation-error-distribution', 'data-savedvalidation-error-density', 'data-savedvalidation-error-coverage', 'data-savedvalidation-account-metrics', 'data-savedvalidation-evidence-json', 'data-savedvalidationscope', 'data-savedvalidationfeature', 'data-savedvalidationcell', 'data-savedvalidation-video-table', 'Registered held-out prediction', 'data-score-axis', 'data-observed-axis', 'other videos from the same creator may remain', 'This is not a forward-time test', 'continue ${unfinished} unfinished', "st.savedChannelSort = 'feature'", 'wireSavedChannelImages', 'record.montageDataUrl = authenticatedMediaUrl(', '`/api/raw/saved-channel/${channelId}/montage/${videoId}`']) {
...
assert(
    auth.includes(
        String.raw`^\/api\/raw\/saved-channel\/ch[a-f0-9]{16}\/(?:video|montage)\/`
    ),
    'saved-channel media must remain in the shared authenticated Experiment route set'
);
const publicAuthBody = auth.slice(
    auth.indexOf('function isPublic('),
    auth.indexOf('// The gate.')
);
assert(
    !publicAuthBody.includes('/api/raw/saved-channel/'),
    'saved-channel media must require account authorization'
);
