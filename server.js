...
Update the idea by calling PATCH /api/data/ideas/${idea.id} with a JSON body containing a "logistics" field with this structure:
{
  ...
  "sourcing_notes": "General sourcing notes for Calgary"
    ...
    if (pathname === '/api/principles/novelty' && req.method === 'GET') {
        try {
            let m = null;
            try { const buf = await cloud.downloadFromR2('raw/principles/novelty.json'); if (buf) m = JSON.parse(buf.toString('utf8')); } catch (e) {}
            res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-cache' });
            res.end(JSON.stringify(m || { error: 'no novelty report yet' }));
        } catch (e) { res.writeHead(500, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ error: e.message })); }
        return;
    }
