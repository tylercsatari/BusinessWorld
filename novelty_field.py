...
def field(Xv):
    """per-video novelty (n,) for each method, on the corpus Xv (normalised)."""
    out = {}
    ...
    for K in [8, 25, 80]:
        cen = norm(MiniBatchKMeans(K, random_state=0, n_init=3, batch_size=1024).fit(Xv).cluster_centers_)
        out[f'niche{K}'] = 1 - (Xv @ cen.T).max(1)
    ...
    mode = Xv[int(np.argmin(dens))]; out['mode'] = 1 - Xv @ mode
    return out
...
for mk, ck in MODK.items():
    ...
    print(f'{mk} ({ck}): {mask.sum()} videos × {len(F)} methods', flush=True)
